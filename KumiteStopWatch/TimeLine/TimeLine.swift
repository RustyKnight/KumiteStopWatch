...
public struct TimeLine {
	...
	public init(withName: String, withDuration: TimeInterval, isPausable: Bool, andEvents:[TimeLineEvent]) {
		self.name = withName
...
public class TimeLineBuilder {
	...
	public init(withName name: String, withDurationOf: TimeInterval, andIsPausable pausable: Bool) {
		self.duration = withDurationOf
		self.name = name
		self.pausable = pausable
	}
	
	public func startWith(color: UIColor, alerts: TimeLineAlert...) -> TimeLineBuilder {
		add(timeLineEvent: TimeLineEvent(location: 0.0, color: color, alerts: alerts))
		return self
	}
	
	public func endWith(color: UIColor, alerts: TimeLineAlert...) -> TimeLineBuilder {
		add(timeLineEvent: TimeLineEvent(location: 1.0, color: color, alerts: alerts))
		return self
	}
	
	public func add(location: Double, color: UIColor, alerts: TimeLineAlert...) -> TimeLineBuilder {
